...
@task
def lint(context: Context, fix: bool = False):
    """Run linters and type checkers"""
    in_py(context, f"hatch run lint:{'fix' if fix else 'all'}")
    in_js(context, "npm run check:format", "npm run check:types")


@task
def test(context: Context, no_cov: bool = False):
    """Run test suites"""
